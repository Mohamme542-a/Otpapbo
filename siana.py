...
# ══════════════════ NumberPanel.tech (المصدر الثالث) 

# ══════════════════ NumberPanel.tech (المصدر الثالث) ══════════════════
# ══════════════════ NumberPanel.tech (التوزيع الذكي للدول حسب الخدمة) ══════════════════
...
# =============== مهمة الحفاظ على الدول ===============
async def maintain_countries(self):
    """
    مهمة خلفية تعمل كل 5 دقائق، تطلب رقماً جديداً من كل دولة موجودة.
    للحفاظ على الدول في قائمة my_numbers.
    """
    try:
        # جلب الدول المحفوظة حالياً
        countries = STATE.get("np_countries", [])
        if not countries:
            return
            
        logging.info(f"NP: Running maintenance for {len(countries)} countries")
        
        # جلب الخدمات المتاحة
        services = list(SERVICE_MAP.keys())
        
        # محاولة طلب رقم من كل دولة
        for c in countries:
            iso = c["iso"]
            name = c["name"]
            # اختيار خدمة عشوائية أو الأولى
            service = services[0] if services else "general"
            
            # محاولة الحجز
            try:
                res = self.request_number(service, iso.upper())
                if res:
                    logging.info(f"NP: Reserved number for {name} ({iso})")
                # انتظار ثانيتين بين الطلبات لتجنب الحظر
                await asyncio.sleep(2)
            except Exception as e:
                logging.warning(f"NP: Failed to reserve for {name}: {e}")
                
    except Exception as e:
        logging.warning(f"NP: maintenance error: {e}")
    ...
    # =============== جلب الدول + الخدمات من الأرقام المحجوزة ===============
    def get_my_countries_with_services(self):
        """
        يجلب الدول مع الخدمات المرتبطة بها من /api/my_numbers.
        """
        found = []
        try:
            ...
            logging.info(f"NP: Found {len(found)} country-service pairs")
        except Exception as e:
...
def main():
    ...
    # =============== تشغيل مهمة الحفاظ على الدول ===============
    scheduler = AsyncIOScheduler()
    scheduler.add_job(NP.maintain_countries, 'interval', minutes=5)
    scheduler.start()
