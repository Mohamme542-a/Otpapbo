...
# في الجروب: الكود يظهر كاملاً بدون نجوم، والرقم فقط يُخفى بالنجوم.
...
REQUIRED_CHANNELS = [
    {"id": -1003974736720, "title": "القناة الأولى", "url": "https://t.me/gvbhvc669"},
]
...
# ══════════════════ Reservations & Last-OTP tracking ══════════════════
# RESERVATIONS: { "<digits-only-number>": {"uid":int, "sid":str, "iso":str,
#                                          "svc_name":str, "country":str, "ts":int} }
RESERVATIONS = {}
# LAST_OTP: dict describing the most recent OTP delivered
LAST_OTP = _load("last_otp.json", None)

def save_last_otp():
    try: _save("last_otp.json", LAST_OTP)
...
def register_reservation(number, uid, sid, iso, svc_name_, country):
    k = _norm_num(number)
    if not k: return
    RESERVATIONS[k] = {"uid": int(uid), "sid": sid, "iso": iso,
                       "svc_name": svc_name_, "country": country,
                       "ts": int(time.time())}

def unregister_reservation(number):
    RESERVATIONS.pop(_norm_num(number), None)

def find_reserver(number):
    return RESERVATIONS.get(_norm_num(number))
...
def note_user(tg_user):
    """يحفظ اسم/يوزر من فتح البوت (لعرضها للأدمن)."""
...
# ══════════════════ Buttons (بدون ألوان — نص عادي) ══════════════════
COLOR_BARS = [""]
SERVICE_COLORS = {}
def color_bar(color, width=3):
    return ""
def bar_label(color, text, width=3):
    return text
...
class NumberPanelSource:
    ...
    # =============== جلب الرسائل من /api/my_otps (الحساب فقط) ===============
    def fetch_otps(self):
        """
        يجلب رسائل OTP من /api/my_otps (نفس النقطة المستخدمة في bot.py من مالك الموقع).
        يعيد قائمة موحّدة الشكل، مع استخراج الكود من الرسالة إن لزم.
        """
        new = []
        try:
            ...
            for it in items:
                if not isinstance(it, dict):
                    continue
                number  = str(it.get("number") or it.get("phone") or "").strip()
                message = str(it.get("message") or it.get("sms") or it.get("text") or "").strip()
                code    = str(it.get("otp_code") or it.get("otp") or it.get("code") or "").strip()
                service = str(it.get("service") or it.get("app") or "").strip()
                country = str(it.get("country") or "").strip()
                ts      = str(it.get("timestamp") or it.get("date") or it.get("time") or "").strip()

                if not number:
                    continue
                # استخرج الكود من نص الرسالة عند غيابه
                ...
                new.append({
                    "id":      uid,
                    "number":  number,
                    "code":    code,          # ← يستخدمها run_session/find_otp_for
                    "otp":     code,
                    "message": message,
                    "date":    ts,
                    "service": service,
            ...
            if new:
                logging.info(f"NP my_otps: {len(new)} new OTP(s)")
        except Exception as e:
    ...
    def get_number(self, rng):
        try:
            ...
            if res:
                return {"number": res["number"], "country": country_name, "iso": iso, "operator": "NumberPanel"}
        except Exception:
            pass
        return None
...
def logins_report():
    ...
    np_ok, np_msg = NP.status()
    try:
        np_ranges = len(NP.ranges())
    except Exception:
        np_ranges = 0
    return (
...
def all_ranges():
    ...
    # المصدر الثالث: NumberPanel — أضف كل الرينجات المتاحة (حتى غير المعروفة مسبقاً)
    try:
...
def find_otp_for(number, seen):
    ...
    # المصدر الثالث: NumberPanel
    try:
        for m in NP.fetch_otps():
...
def main_kb(lang, is_admin):
    rows = [[KeyboardButton(tr(lang, 'get_number'))]]
    row2 = [KeyboardButton(tr(lang, 'language'))]
    if is_admin: row2.append(KeyboardButton(tr(lang, 'admin_panel')))
    rows.append(row2)
...
def countries_kb(sid, lang, page=0):
    ...
    for idx, c in enumerate(sl):
        ...
        # 1. حاول استخراج اسم الدولة من الرينج نفسه (للرينجات اليدوية)
        try:
        ...
        # 2. إذا لم يوجد اسم مكتوب، اكتشف الاسم من رمز ISO تلقائياً (للرينجات التلقائية)
        if not country_display_name:
            country_display_name = iso_name(iso, lang)
            
        # 3. إذا فشل كل شيء، اعرض الرمز نفسه كحل أخير (لكن بشكل نظيف)
        if not country_display_name:
            country_display_name = iso.upper() if iso else "Unknown"
            
        # تنظيف النص من أي فراغات
        country_display_name = country_display_name.strip()
        
        # ✅ بناء الزر بشكل جميل وملوّن (شريط ملوّن يتناسب مع الزر)
        color = COLOR_BARS[idx % len(COLOR_BARS)]
        inner = f"{flag(iso)} {country_display_name} ✅ {c['hits']}"
        label = bar_label(color, inner, width=2)
...
def admin_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🚫 حظر / فك مستخدم", callback_data="adm:ban")],
...
async def cmd_last(update, ctx):
    ...
    if is_admin:
        num = L.get("number") or "—"
        code = L.get("code") or "—"
        user_line = (f"@{L['username']}" if L.get("username")
                     else (L.get("name") or "—")) + f" (<code>{L.get('uid')}</code>)"
    else:
...
async def cmd_pm(update, ctx):
    ...
    if len(args) < 2:
        await update.message.reply_text(
            "الاستخدام:\n<code>/pm &lt;user_id&gt; نص الرسالة</code>",
    ...
    try:
        ...
        await update.message.reply_text(f"✅ أُرسلت إلى <code>{tid}</code>",
                                        parse_mode=ParseMode.HTML)
    except Exception as e:
...
async def send_number(ctx, chat_id, uid, sid, iso, edit_mid=None):
    ...
    if edit_mid:
        ...
        m = await ctx.bot.send_message(chat_id, body, parse_mode=ParseMode.HTML, reply_markup=kb); mid = m.message_id
    # سجّل الحجز حتى نستطيع توجيه الـ OTP لصاحب الرقم
    ...
    try:
        while time.time() < end:
            ...
            if hit:
                ...
                # حفظ آخر OTP للاستعلام عبر /last
                set_last_otp(uid, current["number"], hit["code"],
                             current["svc_name"], current["country"], iso)
                ...
                # Group notice with masked number + masked code + reserver info
                if OTP_GROUP_ID:
                    try:
                        ...
                        who_line = f"👤 <b>سحب بواسطة:</b> @{who}\n" if who else \
                                   f"👤 <b>سحب بواسطة:</b> {u.get('name') or ('ID '+str(uid))}\n"
                        await ctx.bot.send_message(
                            OTP_GROUP_ID,
                            "🔔 <b>OTP وصل</b>\n"
                            "━━━━━━━━━━━━━━━━━━━━━\n"
...
async def on_callback(update, ctx):
    ...
    if data.startswith("adm:") and uid in ADMIN_IDS:
        ...
        if sub == "ban":
            ...
            await q.edit_message_text(
                "🚫 أرسل <b>ID</b> المستخدم لحظره أو فك حظره.\n"
                "(الايدي يظهر بجانب اسم المستخدم في قائمة 👥 مستخدمون)",
                parse_mode=ParseMode.HTML); return
        if sub == "users":
            lines = [f"👥 <b>المستخدمون:</b> {len(USERS)}", "━━━━━━━━━━━━━━━━━━━━━"]
            items = sorted(USERS.items(),
                           key=lambda kv: -(kv[1].get("stats", {}).get("numbers", 0)))
            for k, x in items[:40]:
                st = x.get("stats", {})
                name = x.get("name") or "—"
                ...
                lines.append(
                    f"{ban}<b>{name}</b> {un}\n"
                    f"   🆔 <code>{k}</code> — 📞 {st.get('numbers',0)} • "
                    f"🔑 {st.get('otps',0)} • ❌ {st.get('cancels',0)}")
            txt = "\n".join(lines)
...
# ═══════════════════════════════════════════════════════════════
# ✨ 25 FEATURES BLOCK (added by request)
# ═══════════════════════════════════════════════════════════════
BANS_FILE     = "bans.json"
...
BANS       = _load(BANS_FILE, {})           # {uid: {"ts":..., "reason":...}}
POINTS     = _load(POINTS_FILE, {})         # {uid: int}
FAVS       = _load(FAV_FILE, {})            # {uid: {"services":[], "countries":[]}}
FSETTINGS  = _load(SETTINGS_FILE, {
    "cost_per_number": 0,           # 0 = مجاناً
...
def save_bans():     _save(BANS_FILE, BANS)
def save_points():   _save(POINTS_FILE, POINTS)
def save_favs():     _save(FAV_FILE, FAVS)
def save_fsettings():_save(SETTINGS_FILE, FSETTINGS)

# --- audit log ---
def audit(uid, action, detail=""):
    try:
        with open(AUDIT_FILE, "a", encoding="utf-8") as f:
            f.write(f"{int(time.time())}\t{uid}\t{action}\t{detail}\n")
    except Exception: pass

# --- points / VIP ---
def get_points(uid): return int(POINTS.get(str(uid), 0))
def add_points(uid, n):
    POINTS[str(uid)] = get_points(uid) + int(n); save_points()
def spend_points(uid, n):
    cur = get_points(uid)
    if cur < n: return False
    POINTS[str(uid)] = cur - int(n); save_points(); return True
def is_vip(uid):
    return bool(get_user(uid).get("vip"))

# --- bans ---
def is_banned(uid):
    if str(uid) in BANS: return True
    return bool(get_user(uid).get("banned"))
def ban_user(uid, reason=""):
    BANS[str(uid)] = {"ts": int(time.time()), "reason": reason}; save_bans()
    u = get_user(uid); u["banned"] = True; save_users()
def unban_user(uid):
    BANS.pop(str(uid), None); save_bans()
    u = get_user(uid); u["banned"] = False; save_users()

# --- rate limit (in-memory) ---
_RATE = defaultdict(list)
def rate_ok(uid):
    now = time.time(); window = 60
    q = _RATE[uid] = [t for t in _RATE[uid] if now - t < window]
    if len(q) >= int(FSETTINGS.get("rate_per_minute", 10)): return False
    q.append(now); return True

# --- concurrent limit ---
def user_active_numbers(uid):
    return [n for n, r in RESERVATIONS.items() if r.get("uid") == int(uid)]
def can_reserve_more(uid):
    limit = FSETTINGS["max_concurrent_vip"] if is_vip(uid) else FSETTINGS["max_concurrent_default"]
    return len(user_active_numbers(uid)) < int(limit)

# --- favs ---
def get_favs(uid):
    return FAVS.setdefault(str(uid), {"services": [], "countries": []})

# --- session extension flag ---
# ctx.bot_data[f"extend:{chat_id}"] = extra_seconds
def extend_session(ctx, chat_id, minutes):
    key = f"extend:{chat_id}"
    ctx.application.bot_data[key] = ctx.application.bot_data.get(key, 0) + int(minutes)*60

# ══════════════════ Commands (features 1-25) ══════════════════

# 1) /cancel — إلغاء الأرقام النشطة للمستخدم
async def cmd_cancel(update, ctx):
    uid = update.effective_user.id
    if is_banned(uid): return
    nums = user_active_numbers(uid)
    if not nums:
        await update.message.reply_text("لا يوجد رقم نشط لديك."); return
    for n in nums:
        try: cancel_number(n)
        except Exception: pass
        unregister_reservation(n)
    cancel_task(ctx, update.effective_chat.id)
    audit(uid, "cancel", ",".join(nums))
    await update.message.reply_text(f"❌ أُلغيت {len(nums)} رقم.")

# 2) /extend — تمديد وقت الانتظار
async def cmd_extend(update, ctx):
    uid = update.effective_user.id
    if is_banned(uid): return
    mins = int(FSETTINGS.get("extend_minutes", 10))
    extend_session(ctx, update.effective_chat.id, mins)
    await update.message.reply_text(f"⏳ تم تمديد الجلسة {mins} دقيقة إضافية.")

# 3) /renew — طلب رقم جديد بنفس آخر خدمة/دولة
async def cmd_renew(update, ctx):
    uid = update.effective_user.id
    if is_banned(uid): return
    u = get_user(uid); last = u.get("last_pick")
    if not last:
        await update.message.reply_text("لا يوجد اختيار سابق لتجديده."); return
    sid, iso = last.get("sid"), last.get("iso")
    if not rate_ok(uid):
        await update.message.reply_text("⏱ تجاوزت حد الطلبات في الدقيقة."); return
    if not can_reserve_more(uid):
        await update.message.reply_text("🚫 تجاوزت حد الأرقام المتزامنة."); return
    cancel_task(ctx, update.effective_chat.id)
    task = asyncio.create_task(run_session(ctx, update.effective_chat.id, uid, sid, iso))
    set_task(ctx, update.effective_chat.id, task)

# 4) /mynumbers — أرقامي النشطة
async def cmd_mynumbers(update, ctx):
    uid = update.effective_user.id
    nums = user_active_numbers(uid)
    if not nums:
        await update.message.reply_text("لا يوجد أرقام نشطة."); return
    lines = ["📞 <b>أرقامك النشطة:</b>", "━━━━━━━━━━━━━━━━━━━━━"]
    for n in nums:
        r = RESERVATIONS.get(n, {})
        lines.append(f"• <code>{n}</code> — {r.get('svc_name','?')} • {flag(r.get('iso',''))} {r.get('country','')}")
    await update.message.reply_text("\n".join(lines), parse_mode=ParseMode.HTML)

# 6) /stats — إحصائياتي
async def cmd_stats(update, ctx):
    uid = update.effective_user.id
    u = get_user(uid); st = u.get("stats", {})
    n = st.get("numbers", 0); o = st.get("otps", 0); c = st.get("cancels", 0)
    rate = (o*100/n) if n else 0.0
    await update.message.reply_text(
        "📊 <b>إحصائياتك</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━\n"
        f"📞 أرقام: <b>{n}</b>\n"
        ...
        f"💎 نقاط: <b>{get_points(uid)}</b>\n"
        f"⭐ VIP: <b>{'نعم' if is_vip(uid) else 'لا'}</b>",
        parse_mode=ParseMode.HTML)

# 7) /history — سجل آخر 20 عملية
async def cmd_history(update, ctx):
    uid = update.effective_user.id
    u = get_user(uid); h = list(reversed(u.get("history", [])))[:20]
    if not h:
        await update.message.reply_text("لا يوجد سجل."); return
    lines = ["🗂 <b>آخر 20 عملية:</b>", "━━━━━━━━━━━━━━━━━━━━━"]
    for it in h:
        ts = time.strftime("%m-%d %H:%M", time.localtime(it.get("ts", 0)))
        lines.append(f"• {ts} — {it.get('service','?')} {flag(it.get('iso',''))} "
                     f"<code>{it.get('number','')}</code> → <b>{it.get('otp','')}</b>")
    await update.message.reply_text("\n".join(lines), parse_mode=ParseMode.HTML)
...
async def cmd_top(update, ctx):
    if update.effective_user.id not in ADMIN_IDS:
        await update.message.reply_text("⛔ للأدمن فقط."); return
    items = sorted(USERS.items(),
                   key=lambda kv: -(kv[1].get("stats", {}).get("otps", 0)))[:10]
    lines = ["🏆 <b>أعلى 10 مستخدمين:</b>", "━━━━━━━━━━━━━━━━━━━━━"]
    for i, (k, x) in enumerate(items, 1):
        st = x.get("stats", {})
        name = x.get("name") or f"ID {k}"
        lines.append(f"{i}. {name} — 🔑 {st.get('otps',0)} / 📞 {st.get('numbers',0)}")
    await update.message.reply_text("\n".join(lines), parse_mode=ParseMode.HTML)

# 9) /services_stats — أكثر الخدمات طلباً
async def cmd_services_stats(update, ctx):
    counter = defaultdict(int)
    for x in USERS.values():
        for h in x.get("history", []):
            counter[h.get("service", "?")] += 1
    items = sorted(counter.items(), key=lambda kv: -kv[1])[:15]
    if not items:
        await update.message.reply_text("لا توجد بيانات."); return
    lines = ["📱 <b>أكثر الخدمات طلباً:</b>", "━━━━━━━━━━━━━━━━━━━━━"]
    for s, c in items:
        lines.append(f"• {s}: <b>{c}</b>")
    await update.message.reply_text("\n".join(lines), parse_mode=ParseMode.HTML)

# 10) /countries_stats — أكثر الدول طلباً
async def cmd_countries_stats(update, ctx):
    counter = defaultdict(int)
    for x in USERS.values():
        for h in x.get("history", []):
            counter[(h.get("iso") or "").lower()] += 1
    items = sorted(counter.items(), key=lambda kv: -kv[1])[:15]
    if not items:
        await update.message.reply_text("لا توجد بيانات."); return
    lines = ["🌍 <b>أكثر الدول طلباً:</b>", "━━━━━━━━━━━━━━━━━━━━━"]
    for iso, c in items:
        lines.append(f"• {flag(iso)} {iso_name(iso,'ar')}: <b>{c}</b>")
    await update.message.reply_text("\n".join(lines), parse_mode=ParseMode.HTML)

# 11) /search <name> — بحث عن خدمة
async def cmd_search(update, ctx):
    q = " ".join(ctx.args or []).strip().lower()
    if not q:
        await update.message.reply_text("الاستخدام: /search whatsapp"); return
    rows = []
    for sid, s in SERVICE_MAP.items():
        names = list(s["name"].values()) + [sid] + s.get("keys", [])
        if any(q in str(n).lower() for n in names):
            rows.append([InlineKeyboardButton(f"{s['emoji']} {svc_name(sid,'ar')}",
                                              callback_data=f"svc:{sid}")])
    if not rows:
        await update.message.reply_text("لا نتائج."); return
    await update.message.reply_text("🔎 النتائج:", reply_markup=InlineKeyboardMarkup(rows))

# 13) /fav — إدارة المفضلة
async def cmd_fav(update, ctx):
    uid = update.effective_user.id
    fav = get_favs(uid)
    args = ctx.args or []
    if not args:
        s = ", ".join(fav["services"]) or "—"
        c = ", ".join(fav["countries"]) or "—"
        await update.message.reply_text(
            f"⭐ <b>مفضلتك</b>\n📱 خدمات: {s}\n🌍 دول: {c}\n\n"
            "أوامر:\n"
            "<code>/fav add svc &lt;sid&gt;</code>\n"
            "<code>/fav add co &lt;iso&gt;</code>\n"
            "<code>/fav rm svc &lt;sid&gt;</code>\n"
            "<code>/fav rm co &lt;iso&gt;</code>",
            parse_mode=ParseMode.HTML); return
    if len(args) < 3:
        await update.message.reply_text("صيغة غير صحيحة."); return
    op, kind, val = args[0], args[1], args[2].lower()
    key = "services" if kind == "svc" else "countries"
    lst = fav[key]
    if op == "add" and val not in lst: lst.append(val)
    elif op == "rm" and val in lst: lst.remove(val)
    save_favs(); await update.message.reply_text("✅ تم.")

# 14) /ban <uid>  و  /unban <uid>
async def cmd_ban(update, ctx):
    if update.effective_user.id not in ADMIN_IDS:
        await update.message.reply_text("⛔ للأدمن فقط."); return
    if not ctx.args:
        await update.message.reply_text("الاستخدام: /ban <user_id> [reason]"); return
    tid = re.sub(r"\D", "", ctx.args[0])
    reason = " ".join(ctx.args[1:])
    if not tid: return
    ban_user(int(tid), reason); audit(update.effective_user.id, "ban", tid)
    await update.message.reply_text(f"⛔ حُظر {tid}.")

async def cmd_unban(update, ctx):
    if update.effective_user.id not in ADMIN_IDS:
        await update.message.reply_text("⛔ للأدمن فقط."); return
    if not ctx.args:
        await update.message.reply_text("الاستخدام: /unban <user_id>"); return
    tid = re.sub(r"\D", "", ctx.args[0])
    if not tid: return
    unban_user(int(tid)); audit(update.effective_user.id, "unban", tid)
    await update.message.reply_text(f"✅ فُك حظر {tid}.")

# 17) /broadcast <text> — إعلان لكل المستخدمين
async def cmd_broadcast(update, ctx):
    if update.effective_user.id not in ADMIN_IDS:
        await update.message.reply_text("⛔ للأدمن فقط."); return
    text = " ".join(ctx.args or []).strip()
    if not text:
        await update.message.reply_text("الاستخدام: /broadcast نص الرسالة"); return
    sent = 0; fail = 0
    for k in list(USERS.keys()):
        try:
            await ctx.bot.send_message(int(k), f"📣 {text}"); sent += 1
        except Exception: fail += 1
    audit(update.effective_user.id, "broadcast", f"sent={sent} fail={fail}")
    await update.message.reply_text(f"✅ {sent} — ❌ {fail}")

# 18/19) نقاط
async def cmd_mypoints(update, ctx):
    uid = update.effective_user.id
    await update.message.reply_text(f"💎 رصيدك: <b>{get_points(uid)}</b>",
                                    parse_mode=ParseMode.HTML)

async def cmd_addpoints(update, ctx):
    if update.effective_user.id not in ADMIN_IDS:
        await update.message.reply_text("⛔ للأدمن فقط."); return
    if len(ctx.args or []) < 2:
        await update.message.reply_text("الاستخدام: /addpoints <uid> <n>"); return
    tid = re.sub(r"\D", "", ctx.args[0])
    try: n = int(ctx.args[1])
    except Exception: n = 0
    if not tid or n == 0: return
    add_points(int(tid), n); audit(update.effective_user.id, "addpoints", f"{tid}+{n}")
    await update.message.reply_text(
        f"✅ أُضيف {n} نقطة إلى {tid} (الرصيد: {get_points(int(tid))})")
...
async def cmd_vip(update, ctx):
    if update.effective_user.id not in ADMIN_IDS:
        await update.message.reply_text("⛔ للأدمن فقط."); return
    if len(ctx.args or []) < 2:
        await update.message.reply_text("الاستخدام: /vip <uid> on|off"); return
    tid = re.sub(r"\D", "", ctx.args[0]); mode = ctx.args[1].lower()
    if not tid: return
    u = get_user(int(tid)); u["vip"] = (mode == "on"); save_users()
    audit(update.effective_user.id, "vip", f"{tid}={u['vip']}")
    await update.message.reply_text(f"⭐ VIP لـ {tid}: {u['vip']}")

# 22) /notify on|off
async def cmd_notify(update, ctx):
    uid = update.effective_user.id
    u = get_user(uid)
    if ctx.args and ctx.args[0].lower() in ("on", "off"):
        u["notify_sound"] = (ctx.args[0].lower() == "on"); save_users()
    state = "🔔 مفعّل" if u.get("notify_sound", True) else "🔕 صامت"
    await update.message.reply_text(
        f"الإشعارات: {state}\n(استعمل: /notify on أو /notify off)")

# 25) /reload — تحديث قوائم الدول/الخدمات يدوياً
async def cmd_reload(update, ctx):
    if update.effective_user.id not in ADMIN_IDS:
        await update.message.reply_text("⛔ للأدمن فقط."); return
    await update.message.reply_text("⏳ جاري التحديث...")
    try:
        fn = getattr(NP, "maintain_countries", None)
        if fn:
            if asyncio.iscoroutinefunction(fn): await fn()
            else: await asyncio.to_thread(fn)
        audit(update.effective_user.id, "reload", "ok")
        await update.message.reply_text("✅ تم تحديث القوائم من NumberPanel.")
    except Exception as e:
...
async def cmd_help(update, ctx):
    ...
    lines = [
        ...
        "/start — البدء",
        "/lang — اللغة",
        "/cancel — إلغاء رقمي الحالي",
        "/extend — تمديد وقت الانتظار",
        "/renew — طلب رقم جديد بنفس الاختيار السابق",
        "/mynumbers — أرقامي النشطة",
        "/stats — إحصائياتي",
        "/history — سجل آخر 20 عملية",
        "/services_stats — أكثر الخدمات",
        "/countries_stats — أكثر الدول",
        "/search &lt;اسم&gt; — بحث عن خدمة",
        "/fav — إدارة المفضلة",
        "/mypoints — رصيد نقاطي",
        "/notify on|off — إشعارات صوتية",
        "/last — آخر OTP",
    ]
    if is_admin:
        lines += [
            "──── Admin ────",
            "/admin — لوحة الأدمن",
            "/pm &lt;uid&gt; &lt;نص&gt; — رسالة خاصة",
            "/broadcast &lt;نص&gt; — إعلان لكل المستخدمين",
            "/ban &lt;uid&gt; — حظر",
            "/unban &lt;uid&gt; — فك حظر",
            "/addpoints &lt;uid&gt; &lt;n&gt; — إضافة نقاط",
            "/vip &lt;uid&gt; on|off — VIP",
            "/top — لوحة الشرف",
            "/reload — تحديث القوائم يدوياً",
        ]
    await update.message.reply_text("\n".join(lines), parse_mode=ParseMode.HTML)

# ══════════════════ Hook: تسجيل الاختيار الأخير لكل مستخدم ══════════════════
# نغلّف send_number لتخزين آخر (sid, iso) للاستخدام في /renew وحدود متزامنة
_ORIG_send_number = send_number
async def send_number(ctx, chat_id, uid, sid, iso, edit_mid=None):
    if is_banned(uid):
        try: await ctx.bot.send_message(chat_id, "⛔ محظور.")
        except Exception: pass
        return None
    if not rate_ok(uid):
        try: await ctx.bot.send_message(chat_id, "⏱ تجاوزت حد الطلبات في الدقيقة.")
        except Exception: pass
        return None
    if not can_reserve_more(uid):
        try: await ctx.bot.send_message(chat_id, "🚫 تجاوزت حد الأرقام المتزامنة.")
        except Exception: pass
        return None
    cost = int(FSETTINGS.get("cost_per_number", 0))
    if cost > 0 and not is_vip(uid):
        if not spend_points(uid, cost):
            try: await ctx.bot.send_message(chat_id,
                f"💎 رصيدك غير كافٍ ({get_points(uid)}/{cost}).")
            except Exception: pass
            return None
    u = get_user(uid); u["last_pick"] = {"sid": sid, "iso": iso}; save_users()
    audit(uid, "reserve", f"{sid}/{iso}")
    return await _ORIG_send_number(ctx, chat_id, uid, sid, iso, edit_mid=edit_mid)
...
def main():
    ...
    app.add_handler(CommandHandler("services_stats",  cmd_services_stats))
    app.add_handler(CommandHandler("countries_stats", cmd_countries_stats))
