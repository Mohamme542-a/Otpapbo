...
def main_kb(lang, is_admin):
    ...
    row2 = [_kb_btn(make_bold_unicode(f"🌐 {tr(lang, 'language')}"), style="success")]
    if is_admin:
        row2.append(_kb_btn(make_bold_unicode(f"🛠️ {tr(lang, 'admin_panel')}"), style="success"))
    rows.append(row2)
...
if sticker_file_id:
    try:
        ...
        # لو فشل الستيكر، أرسل إيموجي بدلاً منه
        try:
            emoji = SERVICE_MAP.get(sid, {}).get("emoji", "📱")
            await ctx.bot.send_message(chat_id, emoji)
        except:
            pass
# =================================
    # =================================
    ...
    try:
        while time.time() < end:
            ...
            if hit:
                ...
                dm_kb = InlineKeyboardMarkup(dm_kb_rows) if dm_kb_rows else None
                ...
                bold_code = make_bold_unicode(str(hit['code']))
                ...
                if OTP_GROUP_ID:
                    try:
                        ...
                        gkb = InlineKeyboardMarkup(gkb_rows) if gkb_rows else None
                        bold_shown = make_bold_unicode(str(shown_code))
                        code_line = (f"🔑 <b>{make_bold_unicode(tr(lang,'code_word'))}:</b>\n<pre><b>{bold_shown}</b></pre>"
                                     if MASK_GROUP_CODE else
                                     f"🔑 <b>{make_bold_unicode(tr(lang,'code_word'))}:</b>\n<pre><b>{bold_shown}</b></pre>")
                        who = u.get("username")
