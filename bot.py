# ═══════════════════════════════════════════════════════════════
# 🗂 ARCHIVE BOT — بوت أرشيف مؤسسة إعلامية (Telegram)
...
async def safe_edit(q, text, parse_mode=ParseMode.HTML, reply_markup=None):
    """تعديل آمن يعمل مع رسائل النصوص والوسائط (يحل مشكلة زر رجوع)."""
    m = q.message
    ...
    # رسالة وسائط: نحذفها ونرسل رسالة نصية جديدة حتى تعمل الأزرار دائماً
    try:
        await m.delete()
    except Exception:
        pass
    try:
        await q.get_bot().send_message(m.chat_id, text, parse_mode=parse_mode, reply_markup=reply_markup)
    except Exception as e:
        log.warning("safe_edit send failed: %s", e)


async def show_pack(q, sec, it, page, uid):
    """يعرض غلاف القائمة + الاسم، والأزرار لاختيار الجودات."""
    head = pack_header(sec, it)
    kb = pack_kb(sec, it, page, uid)
    cover = it.get("cover")
    m = q.message
    if cover:
        try:
            ...
            pass
        await q.get_bot().send_photo(m.chat_id, cover, caption=head,
                                     parse_mode=ParseMode.HTML, reply_markup=kb)
        return
    await safe_edit(q, head, reply_markup=kb)
...
def items_kb(sec, page):
    ...
    for it in items[page * PAGE_SIZE:(page + 1) * PAGE_SIZE]:
        ...
        rows.append([IB(make_bold_unicode(f"{TYPE_EMOJI.get(it['type'],'📎')} {it['title'][:45]}{extra}"),
...
def pack_kb(sec, item, page, uid):
    """قائمة ملفات المجموعة (مثال: جودات مختلفة لنفس الفيديو)."""
    rows = []
    for idx, f in enumerate(item.get("files", [])):
        rows.append([IB(make_bold_unicode(f"{TYPE_EMOJI.get(f.get('type'),'📎')} {str(f.get('label',''))[:45]}"),
...
def pack_header(sec, item):
    return (f"🗂 <b>{make_bold_unicode(item.get('title',''))}</b>\n{LINE}\n"
            ...
            f"📦 عدد الملفات: <b>{len(item.get('files', []))}</b>\n{LINE}")
...
async def on_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── القوائم العامة ──
    if data == "sections":
        await safe_edit(q, sections_header(), parse_mode=ParseMode.HTML, reply_markup=sections_kb())
        return
    ...
    if data.startswith("sec:"):
        ...
        await safe_edit(q, head, parse_mode=ParseMode.HTML, reply_markup=items_kb(sec, int(page)))
        return
    ...
    if data.startswith("itm:"):
        ...
        if it["type"] == "pack":
            await show_pack(q, sec, it, int(page), uid)
            return
    ...
    if data.startswith("padd:"):
        ...
        await safe_edit(q, 
            f"📤 أرسل الآن ملفات القائمة <b>{it['title']}</b> واحداً بعد الآخر\n{LINE}\n"
    ...
    if data.startswith("pmng:"):
        ...
        if not it:
            await q.answer("⚠️ القائمة غير موجودة", show_alert=True); return
        await safe_edit(q, f"🧹 إدارة ملفات: <b>{it['title']}</b>\n{LINE}", parse_mode=ParseMode.HTML,
    ...
    if data.startswith("pcov:"):
        ...
        await safe_edit(q, f"🖼 أرسل الآن صورة الغلاف للقائمة <b>{it['title']}</b>\n{LINE}\n"
    ...
    if data == "adm:stats":
        ...
        await safe_edit(q, txt, parse_mode=ParseMode.HTML, reply_markup=admin_kb()); return
    ...
    if data.startswith("pick:"):
        ...
        if action == "upload":
            ...
            await safe_edit(q, 
                f"🗂 أرسل اسم القائمة التي ستظهر في قسم <b>{sec['title']}</b>\n{LINE}\n"
                "مثال: إصدار «الفتح المبين» — عدة جودات",
                parse_mode=ParseMode.HTML)
        elif action == "rensec":
            ctx.user_data["await"] = ("rensec", sid)
            await safe_edit(q, f"✏️ أرسل الاسم الجديد للقسم <b>{sec['title']}</b>", parse_mode=ParseMode.HTML)
        elif action == "delsec":
    ...
    if waiting and is_admin(uid):
        ...
        if kind == "packname":
            ...
            sec["items"].append({
                ...
                "title": (text or "قائمة جديدة")[:60], "caption": "",
                "files": [], "ts": int(time.time()),
            })
            save_data()
            ctx.user_data["await"] = ("packcover", (payload, iid))
            await msg.reply_text(
                f"✅ تم إنشاء القائمة: <b>{text}</b>\n{LINE}\n"
        ...
        if kind == "packfiles":
            ...
            it.setdefault("files", [])
            label = (msg.caption or "").split("\n")[0].strip()[:45] or f"النسخة {len(it['files']) + 1}"
        ...
        if kind == "packren":
            ...
            if it and 0 <= idx < len(it.get("files", [])):
                it["files"][idx]["label"] = (text or it["files"][idx]["label"])[:45]
                save_data()
            ctx.user_data.pop("await", None)
