...
def pack_header(sec, item):
    return (f"🗂 <b>{make_bold_unicode(item.get('title',''))}</b>\n{LINE}\n"
            f"{item.get('caption') or 'اختر الملف الذي تريده 👇'}\n"
...
async def on_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("itm:"):
        ...
        if it["type"] == "pack":
            await q.edit_message_text(pack_header(sec, it), parse_mode=ParseMode.HTML,
                                      reply_markup=pack_kb(sec, it, int(page), uid))
            return
    ...
    if data.startswith("pmng:"):
        ...
        if not it:
            await q.answer("⚠️ القائمة غير موجودة", show_alert=True); return
        await q.edit_message_text(f"🧹 إدارة ملفات: <b>{it['title']}</b>\n{LINE}", parse_mode=ParseMode.HTML,
    ...
    if data.startswith("pren:"):
        ...
        await q.edit_message_text("✏️ أرسل الاسم الجديد لهذا الملف:"); return
    ...
    if data == "adm:pack":
        await q.edit_message_text("🗂 اختر القسم الذي ستوضع فيه القائمة:", reply_markup=pick_section_kb("pack")); return
...
async def on_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if waiting and is_admin(uid):
        ...
        if kind == "packname":
            ...
            iid = next_id("i")
            sec["items"].append({
                "id": iid, "type": "pack", "file_id": None,
                "title": (text or "قائمة جديدة")[:60], "caption": "",
                "files": [], "ts": int(time.time()),
            })
            save_data()
            ctx.user_data["await"] = ("packfiles", (payload, iid))
            await msg.reply_text(
                f"✅ تم إنشاء القائمة: <b>{text}</b>\n{LINE}\n"
                "📤 أرسل الآن ملفاتها واحداً بعد الآخر (فيديو بجودات مختلفة مثلاً)\n"
                "🔸 اكتب اسم الملف في التعليق (مثل: جودة 1080p) أو اتركه فأسميه تلقائياً\n"
                "🔸 عند الانتهاء أرسل /done",
                parse_mode=ParseMode.HTML)
            return
